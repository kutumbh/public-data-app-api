    ...
    try {
        ...
        if (options === "All" && status === "Summary") {
            if (_.isEmpty(surname)) {
                ...
                const kuldevtaFamilyDeity = await surnamesModel.count({kuldevtaFamilyDeity:{$ne:[]}})
                // aggregate([
                //     { $unwind: "$kuldevtaFamilyDeity" }, { $group: { _id: "$kuldevtaFamilyDeity", totalCount: { $sum: 1 } } }])
                const translations = await surnamesModel.count({translations:{$ne:[]}})
                ...
                if (script) {
                    res.status(200).send([
                        { option: { "name": "script", script } }, { option: { "name": "gotra", gotra } },
                        { option: { "name": "community", community } }, { option: { "name": "religion", religion } },
                        { option: { "name": "kuldevtaFamilyDeity", kuldevtaFamilyDeity } }, { option: { "name": "translations", translations } }
                    ])
                } else {
                    res.status(404).send({
        ...
        // -*****************Mother Toung filters***************
        ...
        else if (options === "Script" && status === "Exception Report" && _.isEmpty(surname)) {
            if (_.isEmpty(surname)) {
            ...
            if (surname.indexOf('-') > -1) {
                ...
                getSurname = await surnamesModel.find({ "script": { $eq: [] } }).limit(1000)
            }
            _.forEach(getSurname, function (value) {
                if (value.sStatus === "SN") {
                    value.sStatus = "New"
        ...
        else if (options === "Script" && status === "Non Master" && _.isEmpty(surname)) {
            getSurname = await surnamesModel.aggregate([
                { $unwind: "$script" },
                {
                    $lookup: {
                ...
                {
                    $match: {
                        "script_info.0": { $exists: false },"script": {$ne:null}
                    }
                },
                // {
                //     $group: {
        ...
        // *********Community filters ************
        else if (options === "Community" && !_.isNull(optionsData) && !_.isNull(status) && status === "All") {
            if (!req.body.surname) {
                getSurname = await surnamesModel.find({
                    "$and": [
        ...
        else if (options === "Community" && status === "Exception Report" && _.isEmpty(surname)) {
            if (_.isEmpty(surname)) {
            ...
            if (surname.indexOf('-') > -1) {
                ...
                getSurname = await surnamesModel.find({ "community": { $eq: [] } }).limit(1000)
            }
            _.forEach(getSurname, function (value) {
                if (value.sStatus === "SN") {
                    value.sStatus = "New"
        ...
        else if (options === "Community" && status === "Non Master" && _.isEmpty(surname)) {
            getSurname = await surnamesModel.aggregate([
                {
                    $unwind: "$community"
                },
                ...
                {
                    $match: {
                        "community_info.0": { $exists: false },"community": {$ne:null}
                    }
                }
                // {
                //     $group: {
        ...
        // -***********Religion filters***********
        else if (options === "Religion" && !_.isNull(optionsData) && !_.isNull(status) && status == "All") {
            if (!req.body.surname) {
                getSurname = await surnamesModel.find({
                    "$and": [
                ...
                if (surname.indexOf('-') > -1) {
                    getnames = await surnamesModel.find({
                        "$and": [
                            { "surname": { $regex: "^[" + [surname] + "]" } },
                            { "religion": optionsData },
                            { "sStatus": { $ne: "SI" } }
            ...
            if (_.isEmpty(surname)) {
                getSurname = await surnamesModel.aggregate([
                    { $unwind: "$religion" }, { $group: { _id: "$religion", totalCount: { $sum: 1 } } }])                // let totalSurnames = { totalCount:getSurname };
                console.log(getSurname)
                if (getSurname) {
        ...
        else if (options === "Religion" && status === "Exception Report" && _.isEmpty(surname)) {
            if (_.isEmpty(surname)) {
            ...
            if (surname.indexOf('-') > -1) {
                ...
                getSurname = await surnamesModel.find({ "religion": { $eq: [] } }).limit(1000)
            }
            _.forEach(getSurname, function (value) {
                if (value.sStatus === "SN") {
                    value.sStatus = "New"
        ...
        else if (options === "Religion" && status === "Non Master" && _.isEmpty(surname)) {
            getSurname = await surnamesModel.aggregate([
                {
                    $unwind: "$religion"
                },
        ...
        //**********kuldevtaFamilyDeity filters************* */ 
        else if (options === "Kuldevta" && !_.isNull(optionsData) && !_.isNull(status) && status === "All") {
            if (!req.body.surname) {
                getSurname = await surnamesModel.find({
                    "$and": [
            ...
            if (_.isEmpty(surname)) {
                getSurname = await surnamesModel.aggregate([
                    { $unwind: "$kuldevtaFamilyDeity" }, { $group: { _id: "$kuldevtaFamilyDeity", totalCount: { $sum: 1 } } }])                 // let totalSurnames = { totalCount:getSurname };
                console.log(getSurname)
                if (getSurname) {
                    res.status(200).send([{ option: { "name": "kuldevtaFamilyDeity", getSurname } }])
                } else {
                    res.status(404).send({
                        message: "No Data Found!"
                    })
        ...
        else if (options === "Kuldevta" && status === "Exception Report" && _.isEmpty(surname)) {
            if (_.isEmpty(surname)) {
                getSurname = await surnamesModel.find({ "kuldevtaFamilyDeity": { $eq: [] } })
                // let totalSurnames = { totalCount:getSurname };
                _.forEach(getSurname, function (value) {
                    if (value.sStatus === "SN") {
                        value.sStatus = "Surname New"
            ...
            if (surname.indexOf('-') > -1) {
                ...
                getSurname = await surnamesModel.find({ "kuldevtaFamilyDeity": { $eq: [] } }).limit(1000)
            }
            // let totalSurnames = { totalCount:getSurname };
            _.forEach(getSurname, function (value) {
                if (value.sStatus === "SN") {
        ...
        else if (options === "Kuldevta" && status === "Non Master" && _.isEmpty(surname)) {
            getSurname = await surnamesModel.aggregate([
                {
                ...
                {
                    $match: {
                        "kuldevtaFamilyDeity_info.0": { $exists: false },"kuldevtaFamilyDeity": {$ne:null}
                    }
                },
                // {
                //     $group: {
        ...
        //***********Gotra*************** */
        ...
        else if (options === "Gotra" && !_.isNull(optionsData) && !_.isNull(status) && status === "All") {
            if (!req.body.surname) {
                getSurname = await surnamesModel.find({
                    "$and": [
        ...
        else if (options === "Gotra" && status === "Exception Report" && _.isEmpty(surname)) {
            if (_.isEmpty(surname)) {
            ...
            if (surname.indexOf('-') > -1) {
                ...
                getSurname = await surnamesModel.find({ "gotra": { $eq: [] } }).limit(1000)
            }
            _.forEach(getSurname, function (value) {
                if (value.sStatus === "SN") {
                    value.sStatus = "New"
        ...
        else if (options === "Gotra" && status === "Non Master" && _.isEmpty(surname)) {
            getSurname = await surnamesModel.aggregate([
                {
                    $unwind: "$gotra"
                },
                ...
                {
                    $match: {
                        "gotra_info.0": { $exists: false },"gotra": {$ne:null}
                    }
                },
                // {
                //     $group: {
        ...
        if (options === "Transliterations" && status === "Summary") {
            if (_.isEmpty(surname)) {
                ...
                console.log(getSurname)
                if (getSurname) {
                    res.status(200).send([{ option: { "name": "translations", getSurname } }])
                } else {
                    res.status(404).send({
                        message: "No Data Found!"
                    })
        ...
        else if (options === "Transliterations" && status === "Exception Report") {
            if (_.isEmpty(surname)) {
            ...
            if (status === "Done" && _.isEmpty(surname)) {
                ...
                getSurname = await surnamesModel.find({
                    $or: [{ "translations.lang": { $ne: optionsData } }, { "translations.lang": optionsData }]
                }).limit(1000)
                _.forEach(getSurname, function (value) {
                    if (value.sStatus === "SN") {
                        value.sStatus = "Surname New"
                ...
                if (surname.indexOf('-') > -1) {
                    ...
                    getSurname = await surnamesModel.find({
                        "translations.lang": { $ne: optionsData }, "surname": { "$regex": surname }
                    }).limit(1000)
                    _.forEach(getSurname, function (value) {
                        if (value.sStatus === "SN") {
                            value.sStatus = "Surname New"
        ...
        // **********only surname****************/
        else if (!_.isEmpty(surname) && _.isEmpty(options) && _.isEmpty(status) && _.isEmpty(optionsData)) {
            if (surname.indexOf('-') > -1) {
                getSurname = await surnamesModel.find({
                    "surname": { "$regex": "^[" + [surname] + "]" }
                })
